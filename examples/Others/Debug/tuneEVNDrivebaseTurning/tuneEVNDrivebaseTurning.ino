...
void setup1()
{
    ...
    right.begin();
    drivebase.begin();
    drivebase.setDebug(DEBUG_TURN_RATE);
}
...
void loop()
{
    if (board.buttonRead())
    {
        ...
        if (Serial.available() >= 11)
        {
            ...
            float tr = drivebase.getMaxTurnRate() * (1 * ((float)Serial.read() - 48) + 0.1 * ((float)Serial.read() - 48) + 0.01 * ((float)Serial.read() - 48));
            delay(750);
            drivebase.setTurnRateKp(kp);
            drivebase.setTurnRateKd(kd);
            drivebase.drive(0, tr);
            delay(1500);
            drivebase.stop();
            delay(750);
        }
